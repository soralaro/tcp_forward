...
//short edf;
{
	...
	unsigned long kn[32];
	#else
	unsigned char*pc1m, *pcr;
	unsigned long *kn;

	pc1m = (unsigned char *)malloc(56);
	pcr = (unsigned char *)malloc(56);
	kn = (unsigned long *)malloc(32*sizeof(unsigned long));
	#endif
...
register unsigned long *raw1;
{
	...
	#if 1
	unsigned long *dough = (unsigned long*)malloc(32 * sizeof(unsigned long));
	#else
...
void des2key_de_3(unsigned char *hexkey, short mode)		/* stomps on Kn3 too */
...
register unsigned char *kptr;		/* unsigned char[8] */
{
	...
	cpDkey(savek);
	des2key((unsigned char *)Df_Key, EN0);
...
static void use2key(from)				/* stomps on Kn3 too */
...
register unsigned char *kptr;		/* unsigned char[24] */
{
	...
	cp3key(savek);
	des3key((unsigned char*)Df_Key, EN0);
